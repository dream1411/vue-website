...
export default {
  ...
  methods: {
      ...
      axios
        .get(
          process.env.VUE_APP_API_URL +
            `getKnowledge?page=${this.page - 1}&sort=${this.sort}&category=${this.category}&sizeContents=${this.sizeContents}`,
          {
    ...
    getCategory() {
      axios
        .get(process.env.VUE_APP_API_URL + `category?root=false&enable=true`, {
          headers: { token: localStorage.getItem("id_token") },
        })
        .then((response) => {
